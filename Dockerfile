...
COPY --from=builder /src/cadastrapp/target/cadastrapp-1.9-addon.zip /etc/georchestra/mapfishapp/addons
COPY --from=builder /src/cadastrapp/target/cadastrapp-1.9.war /var/lib/jetty/webapps/cadastrapp.war

RUN unzip /etc/georchestra/mapfishapp/addons/cadastrapp-1.9-addon.zip                                       && \
    unzip -d /var/lib/jetty/webapps/cadastrapp /var/lib/jetty/webapps/cadastrapp.war                        && \
    rm -f /etc/georchestra/mapfishapp/addons/cadastrapp-1.9-addon.zip /var/lib/jetty/webapps/cadastrapp.war && \
